...
# 비양중용 array
ss_array_floor = np.ravel(ss_array)[1:-1].reshape(-1, 2)

# 양중O 이미지프레임 추출
count = 0
# hanging_img_num = datetime.strptime("00:00:00", "%H:%M:%S")
hanging_img_num = 0
delta_dict = list()
for i in ss_array:
    ...
    hanging_img_num += delta_dt_sec


    count += 1

    ##### TODO 2 : choose output image file name #####
    command = "ffmpeg -vsync 2 -ss " + s_time + " -to " + e_time +  " -i '" + input_path + "' -vf thumbnail=" + str(fps) + " 'C:/Users/Snucem_W1/Desktop/VideoToImage/output/변환후 img/O_C" + str(count) + "_%06d.jpg'"
